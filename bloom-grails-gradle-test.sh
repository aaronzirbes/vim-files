...
if [ "${upstream}" == "${mega_repo}" ]; then
    ...
    # Phase (Bloom specific)
    echo "Determining test phase for ${file}"
    phase="test"
    if ( echo "${file}" |grep -q -E '(plugins|webapps)/.*/test/unit/.*bloomhealth.*'); then
        phase="grailsTestUnit"
    elif ( echo "${file}" | grep -q -E '(plugins|webapps)/.*/test/integration/.*bloomhealth.*'); then
